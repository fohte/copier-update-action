...
This action narrows that gap with a multi-stage pipeline:
...
3. For conflict blocks that are flat JSON key/value maps (`package.json` dependencies, scripts, ...) that mergiraf still leaves bundled together, it merges key-by-key: a key changed on only one side is adopted from that side, and only a key changed differently on both sides stays conflicted, isolated into its own marker block.
4. For conflicts still left behind, it compares the `before updating` and `after updating` sides as versions. When both sides parse as a comparable version, it keeps whichever is semantically newer — following the template's bump, or keeping the repository's own newer value instead of downgrading it. Conflicts that aren't a clean version-only difference are left for manual resolution.
...
| `unresolved-files` | newline-separated string (empty when none) | paths of files that still contain conflict markers after mergiraf, the JSON key-level merge, and the version-based resolver have all attempted to resolve them. |
